...
generated quantities {
...

    {
      ...
      for (i in 1:n_weeks) {
        ...
        if (y[i,1] > -1)
          llx[i,1] = neg_binomial_2_lpmf(y[i,1]| min([dC[i],1e8]),phi);
        else
          llx[i,1] =0.;

        if (y[i,2] > -1)
          llx[i,2] = neg_binomial_2_lpmf(y[i,2]| min([dR[i],1e8]),phi);
        else
          llx[i,2] =0.;

        if (y[i,3] > -1)
          llx[i,3] = neg_binomial_2_lpmf(y[i,3]| min([dD[i],1e8]),phi);
        else
          llx[i,3] =0.;
